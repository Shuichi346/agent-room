...
- 2026-05-17 - Conversation response calls should inherit generation defaults from the configured OpenAI-compatible server. The direct HTTP fallback omits `temperature` unless a caller explicitly sets it; free-flow speaker selection and auto-agent generation still set their own low temperatures for control tasks.
- 2026-05-18 - Agent-facing integrations should prefer `POST /api/agents/run` or `python -m backend.app.cli run` when they need a complete JSON response. The existing `/api/chat/run` SSE endpoint remains the UI streaming path.
